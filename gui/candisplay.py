...
# that are used by the gui

# default font and colors
globalfont = "Helvetica 18"
...
class can_dial(clutter.Texture):
	def __init__(self, canbus, canid, sigstartbit, siglength, scalefactor):
		clutter.Texture.__init__(self)
		self.canbus = canbus
		self.canid = canid
		self.sigstartbit = sigstartbit
		self.siglength = siglength
		self.scalefactor = scalefactor
		self.dial_tex = clutter.cogl.texture_new_from_file('/home/eric/uwaft-hmi/gui/img/dial.svg', clutter.cogl.TEXTURE_NO_SLICING, clutter.cogl.PIXEL_FORMAT_ANY)
		self.set_cogl_texture(self.dial_tex)
		self.set_size(120,15)
	
	def set_size(self, width, height):
		# call parent to set size
		clutter.Texture.set_size(self, width, height)
		self.width = width
		self.height = height
