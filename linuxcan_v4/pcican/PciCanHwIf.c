...
static int pciCanTxAvailable (VCanChanData *vChd)
{
    ...
    return ((ioread8(hChd->sja1000 + PCAN_SR) & PCAN_TBS) == PCAN_TBS);
#else
...
static int DEVINIT pciCanProbe (VCanCardData *vCd)
{
...
#endif
#if 0
            if (1 /* DBGF(DEBUG_INIT) */) {
                unsigned int sum = 0, n;
                DEBUGPRINT(1, "EEPROM Channel %d: ", vChd->channel);
                for (n = 0; n < 32; ++n) {
                    DEBUGPRINT(1, "%02x ", buf [n]);
                    sum += buf [n];
                }
                DEBUGPRINT(1, ", sum=%02x\n", sum);
                DEBUGPRINT(1, "%s: channel %d piggy %02x (%d)\n",
    ...
    unsigned int x = hCd->sjaBase;

    for (i = 0; i < 64; i++) {
        printk("<1>%04x  ", x + i*16);
        for (j = 0; j < 16; j++) {
            printk("%02x ", ioread8(x + i*16 + j));
        }
        printk("\n");
    }
    }
...
#if 0
    DEBUGPRINT(3, "pciCanTransmitIsr\n");
#endif
