 ...
 */
static int ds_check_for_presence_single (DALLAS_CONTEXT *dc)
{
  if (!dc->in_mask) {
    return 0;
  }
...
#if 0
  if (stat == dsError_None) {
    PRINTF(("\nds_read_memory_repeat() mask=%x&%x, addr=%x, size=%u ",
            dc->address_in, dc->in_mask, address, bufsiz));
    for (i = 0; i < bufsiz; i++) {
      if (!(i & 0x7)) {
        PRINTF(("\n%4u  ", i));
      }
